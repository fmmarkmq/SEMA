...
SERVER_LOGFILE="rl_grpo_server_qwen14b@llama8_8gpu.log"
CUDA_VISIBLE_DEVICES=7 trl vllm-serve --model files/checkpoints/prefilling_self-tuned-Qwen2.5-14B-Instruct > "$SERVER_LOGFILE" 2>&1 &
VLLM_PID=$!
...
accelerate launch --num_processes 7 \
    ...
    --seed 42 \
    --data_seed 42 \
    --model_name_or_path files/checkpoints/prefilling_self-tuned-Qwen2.5-14B-Instruct \
    --output_dir files/checkpoints/SEMA-Qwen2.5-14B-Instruct@Llama8B \
    ...
    --log_completions \
    --report_to wandb \
    --run_name rl_grpo_qwen14b@llama8_8gpu \
