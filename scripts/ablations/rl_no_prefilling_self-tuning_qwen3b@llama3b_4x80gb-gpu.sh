...
CUDA_VISIBLE_DEVICES=3 trl vllm-serve --model Qwen/Qwen2.5-3B-Instruct > "$SERVER_LOGFILE" 2>&1 &
VLLM_PID=$!
...
accelerate launch --num_processes 3 \
    ...
    --output_dir files/checkpoints/SEMA-Qwen2.5-3B-Instruct@Llama3B_no_prefilling_self-tuning \
    ...
    --log_completions \
    --report_to wandb \
    --run_name rl_grpo_qwen3b@llama3b_4gpu_no_prefilling_self-tuning \
